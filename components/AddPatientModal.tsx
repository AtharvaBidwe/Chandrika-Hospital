    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-white/20">
        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div>
            <h2 className="text-2xl font-black text-slate-900 tracking-tight italic uppercase">Admission Desk</h2>
            <p className="text-[10px] font-bold text-indigo-500 uppercase tracking-[0.2em] mt-0.5">Clinical Operating System</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[80vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              <ActivityIcon />
              <span className="text-xs font-black uppercase tracking-widest">Physio Plan</span>
            </button>
            <button
              type="button"
            ...
            >
              <XrayIcon />
              <span className="text-xs font-black uppercase tracking-widest">X-Ray Order</span>
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input required className={inputClasses} value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Atharva Bidwe" />
            </div>
            <div>
              <label className={labelClasses}>Contact Number</label>
              <input required type="tel" className={inputClasses} value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} placeholder="98XXXXXXXX" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-1">
             ...
             <div className="md:col-span-2">
              <label className={labelClasses}>Town / Area</label>
              <input required className={inputClasses} value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} placeholder="e.g. Pune City" />
            </div>
          </div>
          ...
          {formData.serviceType === 'x-ray' && (
            <div className="space-y-4 animate-in slide-in-from-top-4">
              <label className={labelClasses}>Select Body Parts (Projections)</label>
          ...
          {formData.serviceType === 'physiotherapy' && (
            <div className="space-y-6 animate-in slide-in-from-top-4">
              <div className="bg-indigo-50/50 p-6 rounded-[2rem] border border-indigo-100/50">
                <label className={labelClasses}>Select Visit Schedule (Days)</label>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className={labelClasses}>Treatment Start</label>
                ...
                <div>
                  <label className={labelClasses}>Course Weeks</label>
          ...
          <div className="pt-6 flex gap-4">
            ...
            <button type="submit" className="flex-[2] px-4 py-4 rounded-2xl bg-indigo-600 text-white font-black uppercase tracking-widest text-[11px] hover:bg-indigo-700 transition-all shadow-2xl shadow-indigo-100">Register Admission</button>
          </div>
        </form>
      </div>
    </div>
