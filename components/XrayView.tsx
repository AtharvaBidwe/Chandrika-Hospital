...
import { BoneIcon, CheckCircleIcon, ClockIcon, ActivityIcon, DownloadIcon } from './Icons';
...
const BODY_PARTS = [
  "Chest (PA/Lat)", "Skull", "Cervical Spine", "Thoracic Spine", "Lumbar Spine",
  "Pelvis", "Shoulder (L/R)", "Elbow (L/R)", "Wrist (L/R)", "Hand (L/R)",
  "Hip (L/R)", "Femur (L/R)", "Knee (L/R)", "Ankle (L/R)", "Foot (L/R)",
  "Abdomen", "Sinuses"
];
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mt-1">
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wide">
                Clinician: <span className="text-slate-600">{CLINIC_CONFIG.clinicianName} ({CLINIC_CONFIG.credentials})</span>
              </p>
            </div>
          </div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  >
                    ...
                    {xray.status === step.id && <div className={`w-2 h-2 rounded-full bg-${step.color}-500 ${step.id !== 'reported' ? 'animate-pulse' : ''}`}></div>}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="bg-slate-900 rounded-3xl overflow-hidden shadow-2xl relative min-h-[400px] flex items-center justify-center">
              {xray.imageUrl ? (
                <div className="relative group w-full h-full">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4">
                    <button 
                      ...
                      className="bg-white/20 backdrop-blur-md text-white px-6 py-2 rounded-full font-bold hover:bg-white/30 transition-all border border-white/30"
                    >
                      Replace Image
                    </button>
                  </div>
                  {isAnalyzing && (
                    <div className="absolute inset-0 bg-indigo-900/60 backdrop-blur-sm flex flex-col items-center justify-center text-white p-6 text-center">
                      ...
                      <h3 className="text-xl font-bold mb-2">Analyzing Imaging Data</h3>
                      <p className="text-indigo-100 max-w-xs text-sm">Gemini AI is processing radiographic features...</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="text-center p-12">
                  ...
                  <h3 className="text-xl font-bold text-white mb-2">No Imaging Data</h3>
                  <p className="text-slate-400 mb-8 max-w-sm mx-auto">Upload a medical X-ray scan or use your camera for analysis.</p>
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className="bg-indigo-600 text-white px-8 py-3 rounded-2xl font-bold hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-900/40"
                  >
            ...
            <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div className="flex-1">
                  <h3 className="text-2xl font-bold text-slate-900">Report Generation</h3>
                  <div className="mt-4 flex items-center gap-4">
                    <div className="flex bg-slate-100 p-1 rounded-xl shadow-inner">
                      <button 
                        onClick={() => setLanguage('en')}
                        className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${language === 'en' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}
                      >
                        English
                      </button>
                      <button 
                        onClick={() => setLanguage('mr')}
                        className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${language === 'mr' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}
                      >
                        मराठी
                      </button>
                    </div>
                ...
                {xray.imageUrl && !isAnalyzing && (
                  <button
                    onClick={handleRunAI}
                    className="px-6 py-2.5 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-all flex items-center gap-2 shadow-lg shadow-indigo-100"
                  >
              ...
              {xray.aiReport ? (
                <div className="prose prose-slate max-w-none">
                  <div className="bg-slate-50 p-8 rounded-2xl border border-slate-200 font-mono text-sm whitespace-pre-wrap leading-relaxed shadow-inner">
                    {xray.aiReport}
                  </div>
                ...
                <div className="py-20 text-center bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                  <p className="text-slate-400 font-medium italic">
                    {xray.imageUrl 
                      ? "Imaging captured. Run analysis to create the report."
                      : "Awaiting imaging capture."}
                  </p>
                </div>
              )}
            </div>
